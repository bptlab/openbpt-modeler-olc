...
export default class CustomRenderer extends BaseRenderer {
  ...
  drawShape(parentGfx, element) {
    if (element.type === `${MODELER_PREFIX}:InitialState`) {
      return this.drawInitialState(parentGfx, element);
    } else if (element.type === `${MODELER_PREFIX}:FinalState`) {
      return this.drawFinalState(parentGfx, element);
  ...
  drawInitialState = function (parentGfx, element) {
  ...
  drawFinalState = function (parentGfx, element) {
    const outerCircle = svgCreate("circle");
    svgAttr(outerCircle, {
      cx: element.width / 2,
      cy: element.height / 2,
      r: Math.min(element.width, element.height) / 2,
    ...
    svgAppend(parentGfx, outerCircle);

    const innerCircle = svgCreate("circle");
    svgAttr(innerCircle, {
      cx: element.width / 2,
      cy: element.height / 2,
      r: Math.min(element.width, element.height) / 3,
      fill: "black",
    });
    svgAppend(parentGfx, innerCircle);
